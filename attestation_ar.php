...
if (isset($_SESSION['congidGA'])) {
    ...
    if ($stmt) {
        ...
        if (mysqli_stmt_num_rows($stmt) > 0 && mysqli_stmt_fetch($stmt)) {
            ...
            // Déterminer le genre
            $gender = ($sexe == 'M' || $sexe == 'm') ? 'رجل' 
                     : (($sexe == 'F' || $sexe == 'f') ? 'امرأة' : 'غير محدد');
            
            // Formater la date de naissance
            $date_naissance_formatee = !empty($daten) ? date('Y/m/d', strtotime($daten)) : 'غير مسجل';
            $date_embauche_formatee = !empty($daterec) ? date('Y/m/d', strtotime($daterec)) : 'غير مسجل';
            
            // Nettoyer le nom (enlever les espaces inutiles)
            $nom_complet = htmlspecialchars(trim($nomFr ?? 'غير مسجل'));
            
            // Déterminer le titre (M./Mme/...) en fonction du genre (pour l'affichage)
            $civilite = '';
            if ($sexe == 'M' || $sexe == 'm') {
                $civilite = 'السيد';
            ...
            $poste_occupe = !empty($titre_libelle) ? htmlspecialchars($titre_libelle) : 'غير مسجل';
            
            // Type de contrat
            switch($contrastage){
                case 0:
                    $type_contrat_val = "مترسّم";
                    break;
                case 1:
                    $type_contrat_val = "متربّص";
                    break;
                case 3:
                    $type_contrat_val = "ملحق";
                    break;
                default:
                    $type_contrat_val = "غير محدد";
            }
            
            // Site d'affectation (délégation)
            $site_affectation = !empty($dep) ? htmlspecialchars($dep) : 'غير مسجل';
            
            // CIN pour information supplémentaire
            $cin_val = !empty($cin) ? htmlspecialchars($cin) : 'غير مسجل';
            $adresse_val = !empty($adresse) ? htmlspecialchars($adresse) : 'غير مسجلة';
            
        } else {
            $error = "لم يتم العثور على بيانات للموظفين النشطين.";
        }
        mysqli_stmt_close($stmt);
    } else {
        $error = "خطأ في تحضير الطلب: " . mysqli_error($connection);
    }
    
} else {
    $error = "وصول غير مصرح به. يرجى تسجيل الدخول.";
}

// Liste des signataires possibles
$signataires = [
    ['nom' => '', 'fonction' => 'الرئيس المدير العام'],
    ['nom' => '', 'fonction' => 'مدير الموارد البشرية'],
    ['nom' => '', 'fonction' => 'كاهية مدير الإدارة الفرعيّة للموارد البشريّة و الماليّة'],
    ['nom' => '', 'fonction' => 'المتصرّف المفوّض']
];
?>
...
<div class="attest-card">
  <div class="attest-body">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center;">
      <div>
        <b>عدد : ........../<?php echo date("Y"); ?></b>
      </div>
      <div>
        <strong>قابس :</strong> في <span id="liveDate"></span>
      </div>
    </div>
    ...
    <?php if (isset($error)): ?>
      <div class="error-message">
        ...
        <p style="margin-top: 1rem; font-size: 0.85rem;">يرجى الاتصال بمسؤول النظام.</p>
      </div>
    <?php else: ?>
    
    <div class="intro-text">
      
      إنّي الممضي أسفله،<span class="signataire-fonction-inline" id="signataireFonctionInline"> الرئيس المدير العام</span> للشركة الجهوية للنقل بقابس <strong>'SOTREGAMES'</strong>، أشهد بصحّة المعطيات التالية :
    </div>

    <div class="info-modern-grid">
      <div class="info-row">
        <div class="info-label"><i class="fas fa-user-circle"></i> الاسم واللقب :</div>
        <div class="info-value">
          <?php echo $civilite . ' ' . $nom_complet; ?>
        </div>
      </div>
      <div class="info-row">
        <div class="info-label"><i class="fas fa-id-card"></i> بطاقة التعريف الوطنيّة :</div>
        <div class="info-value"><?php echo $cin_val; ?></div>
      </div>
      <div class="info-row">
        <div class="info-label"><i class="fas fa-calendar"></i> تاريخ الولادة :</div>
        <div class="info-value" dir=rtl><?php echo $date_naissance_formatee; ?></div>
      </div>
      <div class="info-row">
        <div class="info-label"><i class="fas fa-passport"></i> رقم الضمان الاجتماعي :</div>
        <div class="info-value"><?php echo htmlspecialchars($ncnss ?? 'غير مسجل'); ?></div>
      </div>
      <div class="info-row">
        <div class="info-label"><i class="fas fa-briefcase"></i> الرتبة :</div>
        <div class="info-value"><?php echo $poste_occupe; ?></div>
      </div>
      <div class="info-row">
        <div class="info-label"><i class="fas fa-calendar-alt"></i> تاريخ الإنتداب :</div>
        <div class="info-value"><?php echo $date_embauche_formatee; ?></div>
      </div>
      <div class="info-row">
        <div class="info-label"><i class="fas fa-file-contract"></i> الوضعية :</div>
        <div class="info-value"><?php echo $type_contrat_val; ?></div>
      </div>
      <div class="info-row">
        <div class="info-label"><i class="fas fa-building"></i> موقع العمل :</div>
    ...
    <div class="mention-box">
      ...
      <?php
       if ($sexe == 'M' || $sexe == 'm') {
        echo 'سلّمت هذه الشهادة للمعنيّ بالأمر، بطلب منه، للإدلاء بها لدى من يهمّه الأمر.';
       }elseif ($sexe == 'F' || $sexe == 'f')
       {
            echo 'سلّمت هذه الشهادة للمعنيّة بالأمر، بطلب منها، للإدلاء بها لدى من يهمّه الأمر.';
       }
      ?>
     </div>
    </div>
  ...
  <div class="signature-part" id="signaturePart">
    <div class="signataire-nom" id="signataireDisplay">عن/ <span id="signataireFonctionSignature">الرئيس المدير العام</span>
	<p style="padding-left: 40px;">و بإذن منه</p></div>
  </div>
</div>
    ...
    <div class="signataire-selector">
      <label>
        <i class="fas fa-signature"></i> اختر الموقع :
      </label>
...
<script>
  // date du jour format arabe
  const today = new Date();
  const optionsDate = { year: 'numeric', month: 'long', day: 'numeric' };
  const dateElement = document.getElementById('liveDate');
  if (dateElement) {
    dateElement.innerText = today.toLocaleDateString('ar-TN', optionsDate);
  }
