...
<html>
<head>
<title>التصرف في الإجازات</title>
...
<script>
function hided() {
    var str1 = $('#myInput').val();
    console.log('Valeur saisie:', str1); // Debug
    if (str1.length > 1) {
        console.log('Chargement en cours...'); // Debug
        $("#Content").load("load-text290725.php", {'myInput':str1}, function(response, status, xhr) {
            if (status == "error") {
                console.log("Erreur:", xhr.status, xhr.statusText);
            } else {
                $('#Content').show(); // Affiche le contenu après chargement
            }
        });
    } else {
        $('#Content').hide(); // Cache si moins de 2 caractères
    }
}
