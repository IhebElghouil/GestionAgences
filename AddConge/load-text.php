...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
  <style>
    /* Styles pour l'iframe dans la modale */
    .modal-body {
      padding: 0;
	  width: 100%;
    }
...
else
{
 ...
 <script>
  ...
  <script>
document.getElementById('TypeRepos').addEventListener('change', function() {
  const selected = this.selectedOptions[0];
  document.getElementById('TypeReposType').value = selected.dataset.type;
});
</script>
</HEAD>
...
$resultAnnee = $conn->query($sqlAnnee);
...
$NbJourMaladie = 0;
while ($rMaladie = $resultMaladie->fetch_assoc()) {
    $NbJourMaladie = $rMaladie['TotalJoursMaladie'];
}

// Affichage des résultats
$num_rows = $resultFind->num_rows;
if ($num_rows > 0) {
    while ($rNom = $resultFind->fetch_assoc()) {
        $mecano = $rNom['mecano'];
        $nom = $rNom['nom'];
        $datenaissance = $rNom['daten'];
        $daterecrutement = $rNom['daterec'];
        $grade = $rNom['libellet'];
        $nbj1 = $rNom['nbj1'];
        $nbj2 = $rNom['nbj2'];
        $rest = $rNom['rest'];
...
<input type=hidden name="SoldeOutOfMaladie" value=<?php echo $rest; ?>>
...
<input type="text" class="form-control"  style="font-size: 18px;color:red;font-weight: bold;" placeholder="الرصيد الحالي للإجازات......" aria-label="SoldeActuel" aria-describedby="basic-addon2" dir=rtl name="Solde" id="Solde" readonly value="<?php echo ($rest-$NbJourMaladieSoustract); ?>">
...
<option value=0 selected="selected">إجازة سنويّة</option>
			  ...
			  <option value=6 data-type="accident">حادث شغل (أوّلي)</option>
			  <option value=6 data-type="prolongation">حادث شغل (تمديد)</option>
			  <option value=6 data-type="rechute">حادث شغل (إنتكاسة)</option>
...
<input type="hidden" name="TypeReposType" id="TypeReposType">
</div>
...
<input type="text" name="NbJoursResteSolde"  style="font-size: 18px;color:green;font-weight: bold;" id="NbJoursResteSolde"  class="form-control" placeholder="الرّصيد المتبقّي......" aria-label="SoldeRestant" aria-describedby="basic-addon2" dir=rtl readonly value="<?php echo ($rest-$NbJourMaladieSoustract); ?>">
</div>

<div  id="buttonStyle" name="buttonStyle">
<button type=submit class="btn btn-primary" id="SaveData" disabled >حفظ المعطيات</button>
</div>
...
echo '
 ...
 $CtrlDateDebut = date_create($r['datedebut']);; // Assuming $r[1] is a date string
$CurrentDate = new DateTime();

$FinishDateConge=date_create($r['datefin']);
...
$ActualYear = $CurrentDate->format('Y');
$CongeYear= $CtrlDateDebut->format('Y');
if($ActualYear==$CongeYear){
echo '<td><a href="delete.php?id='.$r['id'].'&Nbjours='.$r['nbjours'].'&mecano='.$mecano.'&type=conge" onclick="return confirm(\'؟هل تريد حذف الإجازة حقّا\');"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
  <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z"/>
  <path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z"/>
</svg></a></td>
<td><a href="#" data-bs-toggle="modal" data-bs-target="#externalModal" data-url="update_conge.php?id='.$r['id'].'&DateDebut='.$CtrlDateDebut->format('Y-m-d').'&DateFin='.$FinishDateConge->format('Y-m-d').'&NbJour='.$r['nbjours'].'&mecano='.$mecano.'&RestSolde='.($rest-$NbJourMaladieSoustract).'" ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
...
echo'
	 <td><a href="print.php?id='.$r['id'].'&nom='.$nom.'"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-printer" viewBox="0 0 16 16">
...
<div class="message_box" style="margin:10px 0px;width: 100%;">
  ...
  <thead>
    <tr>
      <th scope="col" width="15%">رقم الراحة</th>
	  <th scope="col" width="10%">نوع الراحة</th>
      <th scope="col" width="15%">بداية الراحة</th>
      <th scope="col" width="13%">إنتهاء الراحة</th>
	  <th scope="col" width="25%">الملاحظات</th>
	  <th scope="col"  width="25%" colspan="2">#</th>
	</tr>
  </thead>
  </table>
  <div class="card-body overflow-scroll" style="width: 100%; height: 500px; align: center;">
  ...
  <tbody class="table-group-divider">
';
while ($rc=$resultAutreConge->fetch_assoc()) {
	  
	 $typeCodes = [
    2  => ['code' => 'CEx',    'typec' => 'راحة إستثنائية'],
    3  => ['code' => 'CِCul',  'typec' => 'رخصة ثقافيّة'],
    4  => ['code' => 'CSy',    'typec' => 'رخصة نقابيّة'],
    5  => ['code' => 'CMa',    'typec' => 'رخصة مرضيّة'],
    6  => ['code' => 'CAt',    'typec' => 'حادث شغل'],
    7  => ['code' => 'CRc',    'typec' => 'راحة تعويضيّة'],
    8  => ['code' => 'CM',    'typec' => 'عطلة أمومة'],
    9  => ['code' => 'CSS',    'typec' => 'عطلة بدون أجر'],
    10 => ['code' => 'CRr',    'typec' => 'إيقاف عن العمل'],
	11 => ['code' => 'CN',    'typec' => 'عطلة ولادة'],
	12 => ['code' => 'CP',    'typec' => 'عطلة أبوّة'],
	13 => ['code' => 'CF',    'typec' => 'دورة تكوينيّة'],
	14 => ['code' => 'CRe',    'typec' => 'إجتماع'],
];

// Cas particulier pour rc[4] == 1
if ($rc['type'] == 1) {
    switch ($rc['type2']) {
        case 1:
            $code = 'CMi.F';
...
} elseif (isset($typeCodes[$rc['type']])) {
    $code  = $typeCodes[$rc['type']]['code'];
    $typec = $typeCodes[$rc['type']]['typec'];
}
    ...
    <td>'.$rc['datedebut'].'</td>
	<td>'.$rc['datefin'].'</td>
	<td width="40%">'.$rc['commentaire'].'</td>';
	
$CtrlDateDebut = date_create($rc['datedebut']);; // Assuming $r[1] is a date string
$CurrentDate = new DateTime();

$FinishDateConge=date_create($rc['datefin']);
...
$CongeYear= $CtrlDateDebut->format('Y');
if($ActualYear==$CongeYear){
echo '<td><a href="delete.php?id='.$rc['id'].'&mecano='.$mecano.'&type=autreconge" onclick="return confirm(\'؟هل تريد حذف الإجازة حقّا\');"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
  <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z"/>
  <path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z"/>
</svg></a></td>
<td><a href="#" data-bs-toggle="modal" data-bs-target="#externalModal" data-url="update_conge.php?id='.$rc['id'].'&DateDebut='.$CtrlDateDebut->format('Y-m-d').'&DateFin='.$FinishDateConge->format('Y-m-d').'&NbJour=0&mecano='.$mecano.'&RestSolde=0" ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
  <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
  <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
</svg></a></td>';
}
...
<td dir=rtl><div class="alert alert-danger" role="alert">
 الرجاء التثبّت من المعلومات !
  ...
  // <iframe src="update_conge.php?id=' . urlencode($r[0]) . '"></iframe>
echo '</table>';
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered" style="max-width: 800px;">
    <div class="modal-content">
	  ...
	  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
        <h5 class="modal-title" id="externalModalLabel">تحيين إجازة العون <?php echo $nom; ?></h5>
        
      </div>
      <div class="modal-body">
        <iframe id="externalFrame" src="" style="width: 100%; height: 600px; border: none;"></iframe>
      </div>
    </div>
  </div>
</div>

  <!-- Scripts JavaScript de Bootstrap 5 -->
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  <script>
    // Lorsque la modale est sur le point d'être affichée
    document.getElementById('externalModal').addEventListener('show.bs.modal', function (event) {
    ...
    // Réinitialiser l'iframe lorsque la modale est fermée
    document.getElementById('externalModal').addEventListener('hidden.bs.modal', function () {
      var iframe = document.getElementById('externalFrame');
      iframe.src = '';
    });
  </script>
